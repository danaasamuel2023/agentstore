 ...
 * NOTE: Announcement content is NOT cached - it's always fetched fresh
 * Only visual design styles are cached for performance
...
export const extractDesignSettings = (store) => {
  ...
  return {
    ...
    // NOTE: announcementPopupStyle is cached but announcement content is NOT
    announcementPopupStyle: store.customization?.announcementPopupStyle || 'none',

    // REMOVED: announcement - now fetched fresh each time, not cached
    // This ensures agents' announcement updates show immediately
...
/**
 * Extract announcement from store data (NOT cached - always fresh)
 * @param {object} store - Full store data from API
 * @returns {object|null} - Announcement settings
 */
export const extractAnnouncement = (store) => {
  if (!store) return null;
...
export const shouldShowAnnouncement = (announcement) => {
  // Must have announcement object and be enabled
  ...
  // Check if showOnce and already shown THIS SESSION
  // Uses a key based on announcement content so new announcements always show
  if (announcement.showOnce && typeof window !== 'undefined') {
    const contentKey = `${announcement.title || ''}_${announcement.message || ''}`.slice(0, 50);
    const shownKey = `announcement_shown_${contentKey}`;
