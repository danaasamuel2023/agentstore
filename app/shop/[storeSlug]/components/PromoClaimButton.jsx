...
export default function PromoClaimButton({ storeSlug }) {
  ...
  const checkVisibility = async () => {
    try {
      ...
      if (data.success && data.data.hasActivePromos && data.data.promoEnabled) {
        setVisible(true);
      }
    } catch {
      // Silently fail
  ...
  const handleClaim = async () => {
    ...
    if (!phone.trim() || phone.trim().length < 10) {
      setResult({ success: false, message: 'Enter a valid phone number' });
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/agent-promo/store/${storeSlug}/promo/claim`, {
        ...
        body: JSON.stringify({ code: code.trim(), phone: phone.trim(), name: name.trim() || undefined })
      });
      const data = await res.json();
  ...
  return (
    <>
      ...
      <div className="fixed top-1/2 -translate-y-1/2 left-4 z-40">
        {pulse && (
          <span className="absolute inset-0 rounded-2xl bg-green-500 animate-ping opacity-30" />
        )}
        <button
          onClick={() => { setShowModal(true); setResult(null); setPulse(false); }}
      ...
      {showModal && (
        <>
          ...
          <div className="fixed inset-0 flex items-start sm:items-center justify-center z-50 pointer-events-none pt-12 sm:pt-0 px-4 overflow-y-auto">
            <div
              className="bg-[#111] w-full sm:max-w-[380px] rounded-2xl pointer-events-auto overflow-hidden mb-4"
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 pt-5 pb-2">
                <div>
                  ...
                  <p className="text-gray-500 text-xs">Enter the promo code to claim</p>
                </div>
                <button
                  onClick={() => setShowModal(false)}
                ...
                {result?.success ? (
                  <div className="py-6 text-center">
                    ...
                    <button
                      ...
                      className="mt-5 px-8 py-2.5 bg-yellow-400 text-black font-bold rounded-xl text-sm"
                    >
                      Done
                    </button>
                  </div>
                    ...
                    <div className="mb-5">
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder="Phone number for data delivery"
                          maxLength={15}
