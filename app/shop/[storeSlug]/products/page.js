...
export default function ProductsPage() {
  ...
  // Function to generate auto email based on phone number
  const generateAutoEmail = (phone) => {
    // Remove any spaces or special characters from phone number
    const cleanPhone = phone.replace(/[\s-]/g, '');
    // Generate email using phone number and a domain
    return `customer_${cleanPhone}@datamartgh.shop`;
  };
  ...
  const handlePurchase = async (product, index) => {
    ...
    if (!customerName) {
      setBundleMessages(prev => ({ 
        ...prev, 
        [index]: { text: 'Please enter your name', type: 'error' } 
      }));
      return;
    }
    
    // Auto-generate email based on phone number
    const autoGeneratedEmail = generateAutoEmail(phoneNumber);
    ...
    try {
      const response = await fetch(`${API_BASE}/agent-stores/stores/${params.storeSlug}/purchase/initialize`, {
        ...
        body: JSON.stringify({
          ...
          phoneNumber,
          customerEmail: autoGeneratedEmail, // Use auto-generated email
          customerName,
          quantity: 1
        })
      });
